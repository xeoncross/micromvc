...
{
	self::$l[$m]=require(SP."modules/$m/lang/$l".EXT);
}
...
{
	//$p=SP.'modules/'.$m.'/lang/';if(class_exists('cookie',0)&&$c=cookie::get('lang')&&is_file($p.$c.EXT))return$c;foreach(self::accepted()as$c)if(is_file($p.$c.EXT))return$c;return'en';
	$p=SP.'modules/'.$m.'/lang/';if(!empty($_COOKIE['lang'])&&strlen($c=$_COOKIE['lang'])==2&&is_file($p.$c.EXT))return$c;foreach(self::accepted()as$c)if(is_file($p.$c.EXT))return$c;return'en';
}
