 ...
 * MB String Functions
 *
 * If the mbstring extension is not enabled/compiled then this class will be 
 * used as a drop-in replacement to emulate those functions.
 *
 * @package		MicroMVC
 * @author		UNKNOWN
 * @copyright	UNKNOWN
...
/**
 * MB alias for compatibility only
 * @param	$str
 * @return	false
 */
function mb_detect_encoding($str) { return false; }


/**
 * MB alias to convert the string to the given encoding using iconv()
 *
 * @todo	iconv seems to still cut strings with bad characters
 * @param	$string the string to convert
 * @param	$to the encoding to use (default UTF-8)
 * @param	$from the strings current encoding (default UTF-8)
 * @return	string
 */
function mb_convert_encoding($string = '', $to = 'UTF-8//IGNORE', $from = 'auto') {
	...
	if( $string ) {
		...
		// Try to use iconv to translate the string
...
* Note: requires mb_strlen amd mb_substr to be loaded
...
* @subpackage strings
*/
function mb_strpos($str, $needle, $offset = NULL) {

    if ( is_null($offset) ) {

        $ar = explode($needle, $str, 2);
        if ( count($ar) > 1 ) {
            return mb_strlen($ar[0]);
        }
        return FALSE;
        ...
        if ( !is_int($offset) ) {
            trigger_error('mb_strpos: Offset must be an integer',E_USER_ERROR);
            return FALSE;
        }
...
* Note: This will get alot slower if offset is used
* Note: requires mb_substr and mb_strlen to be loaded
* @param string haystack
* @param string needle (you should validate this with utf8_is_valid)
...
* @subpackage strings
*/
function mb_strrpos($str, $needle, $offset = NULL) {
    ...
    if ( is_null($offset) ) {
        ...
        if ( !is_int($offset) ) {
            trigger_error('mb_strrpos expects parameter 3 to be long',E_USER_WARNING);
            return FALSE;
        }
