...
{
	if(!preg_match('/[^\x00-\x7F]/S', $t)){$e = error_reporting(~E_NOTICE);
...
{
	preg_match_all('/<(?!\/|a)([^<>]+)\b[^\/|\>]*>/', $h, $o);
preg_match_all('/<\/(?!a)([^<>]+)\b[^\/|\>]*>/', $h, $c);
return(($a = array_diff($c[1], $o[1]))?'<' . join('><', $a) . '>':'') . $h . (($b = array_diff($o[1], $c[1]))?'</' . join('></', array_reverse($b)) . '>':'');
...
{
	return preg_replace('/\[([a-z]+:\/\/(([a-z0-9-]{1, 70}\ . ){1, 4}([a-z]{2, 4})(:\d{2, 4})?(\/[\w\/ . \-? = &;
%]{1, 200})?))\]/i', '<a href = "$1" ' . ($d?' rel = "nofollow"':'') . '>$1</a>', $t);
