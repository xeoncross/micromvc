...
{
	if((error_reporting()&$c)===0)return TRUE;self::$found=1;self::header();$v=new View('error');$v->error=$e;$v->title=lang($c);print $v;log_message("[$c] $e [$f] ($l)");return TRUE;
}


public static function exception(Exception $e)
{
	self::$found=1;$m="{$e->getMessage()} [{$e->getFile()}] ({$e->getLine()})";try{log_message($m);self::header();$v=new View('exception');$v->exception=$e;print$v;}catch(Exception$e){print$m;}exit(1);
}
