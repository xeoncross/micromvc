 ...
 */
public static function file($file, $dir, $overwrite = FALSE, $size = FALSE)
{
	// Invalid upload?
	...
	$filename = string::sanitize_filename($filename);
	...
	if( ! static::allowed_file($extension)) return FALSE;
	
	// Make sure we can use the destination directory
	dir::usable($dir);
	
	// Create a unique name if we don't want files overwritten
	$name = $overwrite ? "$filename.$ext" : self::unique_filename($dir, $filename, $extension);
 ...
 * @return boolean
 */
public static function allowed_file($ext)
{
	if( ! static::$allowed_files) return TRUE;
	return in_array($ext, explode('|', static::$allowed_files));
}
 ...
 * @return string
 */
public static function unique_filename($dir, $file, $ext)
{
